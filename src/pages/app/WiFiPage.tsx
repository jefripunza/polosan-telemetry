...
export default function WiFiPage() {
  ...
  useEffect(() => {
    setAvailableNetworks([
      { ssid: 'Home_WiFi_5G', signal: 85, security: 'wpa2' },
      { ssid: 'Office_Network', signal: 72, security: 'wpa2' },
      { ssid: 'Guest_WiFi', signal: 45, security: 'open' },
      { ssid: 'Neighbor_2.4G', signal: 38, security: 'wpa' },
      { ssid: 'Mobile_Hotspot', signal: 62, security: 'wpa2' },
    ]);

    setSavedNetworks([
      { ssid: 'Home_WiFi_5G', lastConnected: '2024-11-03 10:30', autoConnect: true },
      { ssid: 'Office_Network', lastConnected: '2024-11-02 14:15', autoConnect: false },
      { ssid: 'Coffee_Shop', lastConnected: '2024-11-01 09:45', autoConnect: false },
    ]);
  }, []);
                ...
                <div key={index} className="p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                    <div className="flex items-center space-x-3">
                      <span className="text-2xl">{getSignalIcon(network.signal)}</span>
                      <div>
                        <div className="flex items-center space-x-2">
                          <span className="font-medium text-gray-900">{network.ssid}</span>
                          <span className="text-lg">{getSecurityIcon(network.security)}</span>
                        </div>
                        <div className="flex items-center space-x-2 text-sm text-gray-500">
                          <span>{language === 'id' ? 'Sinyal' : 'Signal'}: {network.signal}%</span>
                          <span>•</span>
                    ...
                    <button
                      onClick={() => handleConnectNew(network.ssid, network.security)}
                      disabled={isConnecting && selectedNetwork === network.ssid}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-500">
                          {language === 'id' ? 'Terakhir terhubung' : 'Last connected'}: {network.lastConnected}
                        </div>
                      </div>
                    </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {language === 'id' ? 'Masukkan Password WiFi' : 'Enter WiFi Password'}
                </h3>
                <button
              ...
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2">
                  {language === 'id' ? 'Jaringan' : 'Network'}: <span className="font-medium">{selectedNetwork}</span>
                </p>
                <input
                  type="password"
                  value={password}
          ...
          <div className="flex items-center space-x-3">
            {connectionStatus === 'connecting' && (
              <>
                ...
                <span className="text-gray-900">
                  {language === 'id' ? 'Menghubungkan ke' : 'Connecting to'} {selectedNetwork}...
                </span>
              </>
            )}
            {connectionStatus === 'success' && (
              <>
                ...
                <span className="text-gray-900">
                  {language === 'id' ? 'Berhasil terhubung ke' : 'Successfully connected to'} {selectedNetwork}
                </span>
              </>
            )}
            {connectionStatus === 'error' && (
              <>
                ...
                <span className="text-gray-900">
                  {language === 'id' ? 'Gagal terhubung ke' : 'Failed to connect to'} {selectedNetwork}
                </span>
              </>
            )}
          </div>
